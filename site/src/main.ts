...
function homePage() {
  ...
  <section class="privacy-block" aria-labelledby="boundary-title"><div class="section-tag">03 · Clear boundary</div><div><h2 id="boundary-title">Your archive is not our dataset</h2><p>The extension stores bookmarks, notes, and decisions in browser storage. A link check contacts only the site for that link. The product has no cloud archive.</p><a class="text-link route-link" href="/privacy">Read the privacy details →</a></div><aside><h3>It does not</h3><ul><li>Upload an archive</li><li>Scrape paywalled pages</li><li>Guess why you saved a link</li><li>Lock export behind payment</li></ul></aside></section>
  <section class="paid" aria-labelledby="paid-title"><div><p class="eyebrow">One-time license</p><h2 id="paid-title">Review a larger archive for $18</h2><p>Free use includes 50 link-check attempts, every note, every decision, and HTML export. One payment removes the limit on this browser.</p></div><div class="purchase-slab"><strong>$18</strong><span>once</span><a class="button primary" href="${CHECKOUT_URL}">Buy the full review</a><button class="button secondary" data-action="paste-license">Have a license? Paste it</button><small>Sociobot and Dodo handle payment and refunds. <a class="route-link" href="/terms">Read the terms.</a></small>${license?.valid ? '<p class="license-ok" role="status">Full review is active on this browser.</p>' : ''}</div></section>`;
}
